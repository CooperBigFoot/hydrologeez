...
def test_hcx_is_only_an_optional_registry_dependency() -> None:
    ...
    assert project["entry-points"]["hcx.models"] == {"dpl_hbv": "hydrologeez.hcx:create_model"}
...
def test_installed_distribution_exposes_dpl_hbv_entry_point() -> None:
    entries = [entry for entry in metadata.distribution("hydrologeez").entry_points if entry.group == "hcx.models"]

    assert [(entry.name, entry.value) for entry in entries] == [("dpl_hbv", "hydrologeez.hcx:create_model")]


def test_entry_point_discovery_does_not_import_hcx() -> None:
    code = textwrap.dedent(
        ...
        entries = [
            ...
            if entry.group == "hcx.models"
        ]
        assert [(entry.name, entry.value) for entry in entries] == [
            ("dpl_hbv", "hydrologeez.hcx:create_model")
        ]
